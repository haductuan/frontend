...
const NewServices = () => {
  ...
  const [allNetwork, setAllNetWork] = useState<Array<any>>([]);
  ...
  const handleConfirm = async () => {
    if (isUnlocked) {
      ...
      setHelperText3("");
      if (!serviceName) {
        setHelperText1("Service name is missing!");
        return;
      }
      if (!network) {
        setHelperText2("Network is missing!");
        return;
      }
      if (!description) {
        setHelperText3("Description is missing!");
        return;
      }
      if (Object.values(requirements).length === 0) {
        enqueueSnackbar(`Please add requirement!`, {
      ...
      try {
        ...
        const res = await zidenPortal.post(
          "/services",
          {
            ...
            network: network,
            requirements: Object.values(requirements).map(
  ...
  React.useEffect(() => {
    fetchAllNetwork();
    fetchAllSchema();
  }, [fetchAllNetwork, fetchAllSchema]);
  React.useEffect(() => {
                    ...
                    <Grid item xs={12} md={6} lg={6}>
                      ...
                      {requirements[requirementId]?.schema && (
                        ...
                        >
                          ...
                          {requirementOptions[requirementId]?.property
                            .length === 0 && (
                            <Box>
                      ...
                      {requirements[requirementId]?.schema &&
                        ...
                        requirements[requirementId]?.comparator !== 0 && (
                          <ComparedValue
                            ...
                            helperText={
                              helperTexts[requirementId].comparedValue
                            }
                          />
                        )}
                    </Grid>
                        ...
                        <Autocomplete
                          ...
                          renderInput={(params) => (
                            <TextField
                              {...params}
                              helperText={
                                helperTexts[requirementId].allowedIssuer
                              }
